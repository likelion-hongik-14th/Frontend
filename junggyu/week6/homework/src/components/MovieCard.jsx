const MovieCard = ({movie,onOpenModal }) => {
    ...
    return(
        <section onClick={()=> onOpenModal && onOpenModal(movie) } className="w-full rounded-lg bg-gray-800 cursor-pointer" >

            <section className="w-full bg-gray-800 p-6 rounded-lg shadow  hover:-translate-y-2 hover:bg-gray-700">
                <img
                    src={movie.image?.medium ?? movie.imageUrl ?? 'https://via.placeholder.com/210x295?text=No+Image'}
